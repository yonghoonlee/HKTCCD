...
prob.model.add_constraint('duct_contraction_ratio_rate', lower=-0.1, upper=0.1)
...
duct_contraction_ratio = prob.get_val('duct_contraction_ratio')
pitch = prob.get_val('pitch')
duct_contraction_ratio_rate = prob.get_val('duct_contraction_ratio_rate')

R = np.zeros((power.shape[0], 6))
R[:,0] = power
R[:,1] = omega
R[:,2] = V_throat
R[:,3] = duct_contraction_ratio
R[:,4] = pitch
R[:-1, 5] = duct_contraction_ratio_rate

current_path = os.path.abspath(os.path.curdir)
with open(os.path.join(current_path, 'result_study02.csv'), 'wt', newline='') as fid:
    writer = csv.writer(fid)
    writer.writerow(['power', 'omega', 'V_throat', 'duct_CR', 'pitch', 'duct_CR_rate'])
    for idx in range(R.shape[0]):
        writer.writerow(R[idx,:].tolist())
